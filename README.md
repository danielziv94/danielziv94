# Hi there, I'm Daniel Ziv 👋

### RPA Team Leader | AI & Automation Architect

I am a results-driven **RPA Team Leader** specializing in the **UiPath** ecosystem, with a passion for bridging the gap between traditional automation and Modern AI. Currently, I'm focusing on building **Agentic AI workflows** and autonomous systems to solve complex business challenges.

---

### 🚀 What I'm working on
* 🤖 **AI Agents:** Developing autonomous frameworks to automate end-to-end development cycles.
* 🛠️ **Enterprise RPA:** Designing scalable, on-premise infrastructure for large-scale automation.
* ☁️ **Cloud Expansion:** Diving deep into AWS to power next-gen automation.

### 🛠️ Tech Stack
* **Automation:** UiPath (Studio, Orchestrator, Document Understanding, Apps).
* **AI & Data:** Python, LLMs, RAG Architectures, Agentic AI.
* **Languages:** Python, C#, SQL.
* **Infrastructure:** AWS (Learning), Docker, Git.

---

### 📊 Presence & Activity
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/daniel-ziv-8139561b2/)
[![Portfolio](https://img.shields.io/badge/Projects-AI_Agents-FF6F00?style=for-the-badge&logo=google-assistant)](https://github.com/danielziv94?tab=repositories)
[![Email](https://img.shields.io/badge/Email-Contact_Me-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:danielziv94@gmail.com)

---

### ⚡ Quick Stats
* 💻 **Main Stack:** Python & UiPath
* 🚀 **Current Focus:** Agentic AI Automation
* 📁 **Status:** Exploring the intersection of RPA and LLMs
